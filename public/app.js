...
function renderPositions() {
  ...
  positions.forEach((position) => {
    ...
    const breakEven = Number(position.quantity) > 0 ? Number(position.costBasisUsd || position.investedUsd) / Number(position.quantity) / (1-state.slippagePct/100) / (1-state.feePct/100) : null;
    row.innerHTML = `<div class="row-between"><div class="asset-name"><strong></strong><span></span></div><div class="position-price"><strong>${unavailable ? 'Price unavailable' : price(position.currentPriceUsd)}</strong><span class="updated">${position.priceUpdatedAt ? relativeTime(position.priceUpdatedAt) : ''}</span></div></div><div class="position-grid position-market"><div><span>Invested</span><strong>${money(position.investedUsd)}</strong></div><div><span>Current value</span><strong>${money(position.currentValueUsd)}</strong></div><div><span>Average entry</span><strong>${price(position.averageEntryPriceUsd || position.entryPriceUsd)}</strong></div><div><span>Break-even</span><strong>${price(breakEven)}</strong></div><div><span>Market cap</span><strong>${compact(position.marketCapUsd)}</strong></div><div><span>Liquidity</span><strong>${compact(position.liquidityUsd)}</strong></div><div><span>24h volume</span><strong>${compact(position.volume24hUsd)}</strong></div><div><span>Data source</span><strong>${position.source || 'Live market'}</strong></div></div><div class="allocation"><span>Portfolio allocation</span><strong>${allocation.toFixed(1)}%</strong><i><b style="width:${Math.min(100, allocation)}%"></b></i></div><div class="position-footer"><span class="pnl ${tone(position.unrealizedPnlUsd)}">${unavailable ? 'Waiting for price' : `${signedMoney(position.unrealizedPnlUsd)} (${percent(position.unrealizedPnlPct)})`}</span><span><button class="text-button copy-position" type="button">Copy CA</button><button class="text-button exits" type="button">Manage exits</button><button class="sell-button" type="button" ${unavailable ? 'disabled' : ''}>Review sell</button></span></div>`;
...
async function loadOrders() {
  try {
    ...
    $('cancelAllOrders').classList.toggle('hidden', !orders.length);
    ...
    orders.forEach(order => {
      ...
      const label = order.type === 'stop_loss' ? 'Stop loss' : order.type === 'take_profit' ? 'Take profit' : 'Trailing stop';
      const detail = order.type === 'trailing_stop' ? `${order.percentToSell}% · trails ${Number(order.trailingPercent).toFixed(1)}% · trigger ${price(order.triggerPriceUsd)}` : `${order.percentToSell}% at ${price(order.triggerPriceUsd)}`;
      row.innerHTML = `<div><span class="order-type ${order.type}">${label}</span><strong></strong><small>${detail} · ${dateTime(order.createdAt)}</small></div><span class="order-actions"><button class="text-button edit-order" type="button">Edit</button><button class="text-button cancel-order" type="button">Cancel</button></span>`;
      row.querySelector('strong').textContent = `${order.symbol || order.tokenName || 'Token'} · ${chainName(order.chain)}`;
      row.querySelector('.edit-order').onclick = () => editOrder(order);
      row.querySelector('.cancel-order').onclick = async () => { await api(`/api/orders/${order.id}`, { method: 'DELETE' }); await loadOrders(); toast('Exit order cancelled.'); };
      list.appendChild(row);
    });
  } catch (_) {}
}

function percentOptions(selected=100){return [25,50,75,100].map(value=>`<option value="${value}" ${Number(selected)===value?'selected':''}>Sell ${value}%</option>`).join('');}
function editOrder(order){const trailing=order.type==='trailing_stop';openModal('Edit exit order',`${trailing?'':`<label class="form-label">Trigger price</label><input id="editTrigger" class="modal-input" type="number" step="any" value="${order.triggerPriceUsd}">`}${trailing?`<label class="form-label">Trailing distance</label><div class="input-suffix"><input id="editTrailing" class="modal-input" type="number" min="0.1" max="90" step="0.1" value="${order.trailingPercent}"><span>%</span></div>`:''}<label class="form-label spaced-label">Position to sell</label><select id="editPercent" class="modal-input">${percentOptions(order.percentToSell)}</select>`,async()=>{await api(`/api/orders/${order.id}`,{method:'PUT',body:JSON.stringify({triggerPriceUsd:trailing?order.triggerPriceUsd:Number($('editTrigger').value),trailingPercent:trailing?Number($('editTrailing').value):null,percentToSell:Number($('editPercent').value)})});await loadOrders();toast('Exit order updated.');},'Save changes');}
function cancelAllOrders(){openModal('Cancel all exit orders','<p class="trade-note">Cancel every active stop-loss, take-profit and trailing-stop order in this portfolio?</p>',async()=>{await api('/api/orders',{method:'DELETE'});await loadOrders();toast('All exit orders cancelled.');},'Cancel all orders');}
async function riskControls(){try{const {settings={}}=await api('/api/risk-settings');openModal('Portfolio risk rules',`<label class="form-label">Maximum size for one position</label><div class="input-suffix"><input id="maxPositionPct" class="modal-input" type="number" min="1" max="100" step="1" value="${Number(settings.maxPositionPct||100)}"><span>%</span></div><label class="form-label spaced-label">Daily realized-loss limit</label><div class="input-prefix"><span>$</span><input id="dailyLossLimit" class="modal-input" type="number" min="0" step="1" value="${Number(settings.dailyLossLimitUsd||0)}"></div><p class="trade-note">Set the loss limit to 0 to disable it. When a rule is reached, new buys are blocked until the next UTC day.</p>`,async()=>{await api('/api/risk-settings',{method:'PUT',body:JSON.stringify({maxPositionPct:Number($('maxPositionPct').value),dailyLossLimitUsd:Number($('dailyLossLimit').value)})});toast('Risk rules saved.');},'Save rules');}catch(error){toast(error.message,true);}}
...
function bindEvents() {
  ...
  $('cancelAllOrders').addEventListener('click', cancelAllOrders);
  $('riskControlsBtn').addEventListener('click', riskControls);
...
function setExit(position){openModal(`Manage exits for ${position.symbol}`,`<div class="exit-grid"><label><span class="form-label">Stop-loss price</span><input id="stopPrice" class="modal-input" type="number" step="any" placeholder="Optional"></label><label><span class="form-label">Take-profit price</span><input id="takePrice" class="modal-input" type="number" step="any" placeholder="Optional"></label></div><label class="form-label spaced-label">Trailing stop distance</label><div class="input-suffix"><input id="trailPercent" class="modal-input" type="number" min="0.1" max="90" step="0.1" placeholder="Optional"><span>%</span></div><label class="form-label spaced-label">Position to sell when triggered</label><select id="exitPercent" class="modal-input">${percentOptions(100)}</select><p class="trade-note">Saving the same order type updates the existing order instead of creating a duplicate.</p>`,async()=>{const stop=Number($('stopPrice').value),take=Number($('takePrice').value),trail=Number($('trailPercent').value),sellPercent=Number($('exitPercent').value);if(!(stop>0)&&!(take>0)&&!(trail>0))throw new Error('Enter at least one exit rule.');const requests=[];if(stop>0)requests.push(api('/api/orders',{method:'POST',body:JSON.stringify({positionId:position.id,type:'stop_loss',triggerPriceUsd:stop,percentToSell:sellPercent})}));if(take>0)requests.push(api('/api/orders',{method:'POST',body:JSON.stringify({positionId:position.id,type:'take_profit',triggerPriceUsd:take,percentToSell:sellPercent})}));if(trail>0)requests.push(api('/api/orders',{method:'POST',body:JSON.stringify({positionId:position.id,type:'trailing_stop',trailingPercent:trail,percentToSell:sellPercent})}));await Promise.all(requests);await loadOrders();toast('Exit orders saved.');},'Save exit rules');}
